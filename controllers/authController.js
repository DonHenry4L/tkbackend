...
exports.userRegister = (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    if (password && confirmPassword && password !== confirmPassword) {
      error.push("Password and Confirm Password Mismatched");
    }
    if (password && password.length < 6) {
      error.push("Password must be six(6) characters long");
    }
    ...
    try {
      ...
      if (checkUser) {
        ...
        if (matchPassword) {
          ...
          res.status(200).cookie("chatAuthToken", token, options).json({
